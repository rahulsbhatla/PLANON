...
public class Main {
	...
	public static void salaries(Database db, Scanner in) {
		...
		System.out.println("6-Print Salaries of all Employees");
		...
		case 1:
			...
			if (current2.getType() == "permanent")
			{
			 ...
			 if(current2.getType().equalsIgnoreCase("permanent"))
						...
						if(salaryafterleaves>15)
						{
							System.out.println("Exhausted all Leaves alloated.No more leaves allowed for this year");
						}
		...
		if (type == "permanent") {
			...
			System.out.println("Amount to be Paid: " + c.amount(current.getNoofdays1()));
		}
		break;
	    ...
	    case 4:
			...
			{
				int sum=0;
				System.out.println("Enter the month you want to know: ");
				int m4 = in.nextInt();
				 for(int m=1;m<=m4;m++)
				 {
				if (current4.getType() == "contract")
				{
					sum = sum+(current4.getNoofdays1()*(8*200));
				}
			}
				 System.out.println("Total salary paid until now to "+current4.getName() +" is "+sum );
			}
